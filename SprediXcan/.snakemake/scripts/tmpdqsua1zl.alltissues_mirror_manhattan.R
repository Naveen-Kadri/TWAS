...
snakemake <- Snakemake(
    ...
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'mem_mib', 'disk_mb', 'disk_mib', 'tmpdir', 'walltime', "mem_mb" = 8000, "mem_mib" = 7630, "disk_mb" = 1000, "disk_mib" = 954, "tmpdir" = '/scratch/257326015.tmpdir', "walltime" = '01:00'),
...
## twas_file <- '/cluster/work/pausch/naveen/TWAS/splicing/SprediXcan/nonadditive/tail/result_with_cords.txt.gz'
## inheri = 'nonadditive'
## maf_thresh <- 0.005
## thresh_line <- 6
...
for (ti in 1:length (tissues) ) {
    ...
    if (nrow (peaks) >0 ) {
        text (peaks$newpos, -1*-log10(peaks[,5]), pos=positions[ti], peaks$gene_name, col=peaks$col, font=2)
    }
}
