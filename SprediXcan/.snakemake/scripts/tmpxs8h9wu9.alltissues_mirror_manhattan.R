...
snakemake <- Snakemake(
    ...
    threads = 1,
    log = list(),
    resources = list('mem_mb', 'mem_mib', 'disk_mb', 'disk_mib', 'tmpdir', 'walltime', "mem_mb" = 8000, "mem_mib" = 7630, "disk_mb" = 1000, "disk_mib" = 954, "tmpdir" = '/scratch/257349428.tmpdir', "walltime" = '01:00'),
...
##for (ti in 1) {
    ...
    ## if (mpheno == "splicing") {
    ##     gene_ids <- sapply (strsplit (ft$gene_name, "_"), "[", 1)
    ##     gene_names<-sapply (strsplit (ft$gene_name, "_"), "[", 2)
    ...
    if (nrow (peaks) >0 ) {
        ...
        PEAKS [[ti]] <- peaks
        text (peaks$newpos, -1*-log10(peaks[,5]), pos=positions[ti], peaks$gene_name, col=peaks$mycol, font=2)
